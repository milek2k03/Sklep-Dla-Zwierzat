-- Remove ShipX automation while preserving existing orders and tracking data.
-- Run after 027_inpost_shipx_integration.sql.

drop function if exists public.claim_inpost_shipment_creation(uuid);

drop index if exists public.orders_inpost_shipment_id_unique_idx;

alter table public.orders
  drop column if exists pickup_point_name,
  drop column if exists pickup_point_address_line1,
  drop column if exists pickup_point_address_line2,
  drop column if exists inpost_shipment_id,
  drop column if exists inpost_shipment_status,
  drop column if exists inpost_service,
  drop column if exists inpost_shipment_error,
  drop column if exists inpost_shipment_started_at,
  drop column if exists inpost_shipment_created_at;
