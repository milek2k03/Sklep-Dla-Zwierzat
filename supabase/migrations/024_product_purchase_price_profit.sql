-- Track product purchase costs and freeze them on order items for profit/loss reporting.
-- Run after 023_return_item_conditions.sql.
...
add column if not exists unit_purchase_price numeric(10, 2) not null default 0
check (unit_purchase_price >= 0);

alter table public.order_items
add column if not exists purchase_total numeric(10, 2) not null default 0
check (purchase_total >= 0);

update public.order_items as order_item
set
  unit_purchase_price = coalesce(product.purchase_price, 0),
  purchase_total = round(coalesce(product.purchase_price, 0) * order_item.quantity, 2)
from public.products as product
where product.slug = order_item.product_slug
  and order_item.unit_purchase_price = 0
  and order_item.purchase_total = 0;
