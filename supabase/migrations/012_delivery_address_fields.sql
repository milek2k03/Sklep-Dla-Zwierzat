-- Separate Polish delivery address fields.
-- Run after 011_order_events.sql.

alter table public.orders
  add column if not exists delivery_city text,
  add column if not exists delivery_street text,
  add column if not exists delivery_building_number text,
  add column if not exists delivery_postal_code text,
  add column if not exists delivery_country text not null default 'Polska';

alter table public.orders
  drop constraint if exists orders_delivery_country_check,
  add constraint orders_delivery_country_check
  check (delivery_country = 'Polska');
...
alter table public.orders
  ...
  check (
    ...
    or delivery_postal_code ~ '^[0-9]{2}-[0-9]{3}$'
  );
